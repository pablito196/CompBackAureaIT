...
export default {
    ...
    list: async (req,res,next) =>{
        try {
            ...
            const reg = await models.Usuario.find({$or:[{'nombre':new RegExp(valor,'i')},{'email':new RegExp(valor,'i')}]})
            res.status(200).json(reg);
        } catch (e) {
            res.status(500).send({
                message:'Ocurrio un error'
    ...
    update: async (req,res,next) =>{
        try {
            ...
            const reg = await models.Usuario.findByIdAndUpdate({_id:req.body._id},{descripcion:req.body.descripcion});
            res.status(200).json(reg);
        } catch (e) {
            res.status(500).send({
                message:'Ocurrio un error'
    ...
    login: async (req,res,next) => {
        try {
            ...
            if(user){
                ...
                if(match) {
                    let tokenReturn = await token.encode(user._id,user.rol,user.email,user.empresa);
